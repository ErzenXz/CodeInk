...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codeink-find")
  api.delete("codeink-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codeink-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codeink-find", new Highlight(...rest))
    return true
  }
