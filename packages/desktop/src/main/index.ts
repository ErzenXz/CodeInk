...
const APP_NAMES: Record<string, string> = {
  dev: "CodeInk Dev",
  beta: "CodeInk Early Access",
  prod: "CodeInk",
}
const APP_IDS: Record<string, string> = {
  dev: "app.codeink.desktop.dev",
  beta: "app.codeink.desktop.early-access",
  prod: "app.codeink.desktop",
}
const TEST_ONBOARDING = process.env.OPENCODE_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = "external-agents"
...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "app.codeink.desktop.dev"
  ...
  app.setPath(
    "userData",
    process.env.CODEINK_DATA_DIR ||
      (onboardingTestRoot ? join(onboardingTestRoot, "desktop") : join(app.getPath("appData"), app.getName())),
  )
  if (onboardingTestRoot) app.setPath("sessionData", join(onboardingTestRoot, "session"))
  if (process.env.CODEINK_DATA_DIR) app.setPath("sessionData", join(app.getPath("userData"), "session"))
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codeink://") || arg.startsWith("opencode://"))
    if (urls.length) {
