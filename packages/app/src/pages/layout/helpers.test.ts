...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codeink://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
    expect(parseDeepLink("opencode://open-project?directory=/tmp/legacy")).toBe("/tmp/legacy")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codeink://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codeink://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codeink://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codeink://open-project")).toBeUndefined()
    expect(parseDeepLink("codeink://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codeink://open-project?directory=/a",
      "codeink://other?directory=/b",
      "codeink://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codeink://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codeink://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codeink://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codeink://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codeink://new-session?directory=/a",
      "codeink://open-project?directory=/b",
      "codeink://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODEINK__: {
        deepLinks: ["codeink://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODEINK__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codeink://open-project?directory=/a"])
