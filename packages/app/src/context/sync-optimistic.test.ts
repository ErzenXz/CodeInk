...
test("confirms a server message by ID even when its timestamp differs from the optimistic message", () => {
  const page = mergeOptimisticPage({ session: [userMessage("msg_same", "ses_1", 200)], part: [], complete: true }, [
    { message: userMessage("msg_same", "ses_1", 100), parts: [] },
  ])
  expect(page.session).toHaveLength(1)
  expect(page.session[0].time.created).toBe(200)
  expect(page.confirmed).toEqual(["msg_same"])
})
