...
import type { Message } from "@codeink/sdk/v2/client"
...
test("keeps model identity when the agent does not report tokens", () => {
  const message = assistant("a", { input: 0, output: 0, reasoning: 0, read: 0, write: 0 }, 0, "local-codex", "gpt-test")
  const ctx = getSessionContext([message], [{ id: "local-codex", name: "Codex", models: {} }])
  expect(ctx?.providerLabel).toBe("Codex")
  expect(ctx?.modelLabel).toBe("gpt-test")
})

test("external usage preserves unknown values and uses the reported context window", () => {
  const message = {
    ...assistant("a", { input: 0, output: 0, reasoning: 0, read: 0, write: 0 }, 0),
    codeinkUsage: { contextUsed: 1250, contextLimit: 10000 },
  }
  const ctx = getSessionContext([message])
  expect(ctx?.usage).toBe(13)
  expect(ctx?.total).toBe(1250)
