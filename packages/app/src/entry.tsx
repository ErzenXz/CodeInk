...
const getCurrentUrl = () => {
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_CODEINK_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_CODEINK_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) => i.name !== "Breadcrumbs" && !(import.meta.env.CODEINK_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
