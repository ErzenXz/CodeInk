...
`@codeink/schema` owns browser-safe wire and storage contracts shared by protocol, server, core, and generated SDKs. Keep runtime behavior, service layers, side effects, and host-local implementation details in the domain package that owns them.

## Package Boundary

- If Protocol and Server packages are present, preserve the dependency direction: `@codeink/schema <- @codeink/protocol <- @codeink/server`.
...
- If present, `@codeink/protocol` and `@codeink/sdk-next` are the current `/api/...` surfaces.
