...
const STORAGE_KEYS = {
  THEME_ID: "codeink-theme-id",
  COLOR_SCHEME: "codeink-color-scheme",
  THEME_CSS_LIGHT: "codeink-theme-css-light",
  THEME_CSS_DARK: "codeink-theme-css-dark",
} as const

const THEME_STYLE_ID = "codeink-theme"
...
function read(key: string) {
  ...
  try {
    return localStorage.getItem(key) ?? localStorage.getItem(key.replace("codeink-", "opencode-"))
  } catch {
    return null
  }
}
...
function write(key: string, value: string) {
  ...
  try {
    ...
    localStorage.removeItem(key.replace("codeink-", "opencode-"))
  } catch {}
}
...
function applyThemeCss(theme: DesktopTheme, themeId: string, mode: "light" | "dark") {
  ...
  document.getElementById("codeink-theme-preload")?.remove()
  ...
  init: (props: {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "codeink" && !knownThemes().has(next) && !store.themes[next]) return
    ...
    onMount(() => {
      ...
      if (rawTheme && rawTheme !== savedTheme) clear()
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "codeink" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
